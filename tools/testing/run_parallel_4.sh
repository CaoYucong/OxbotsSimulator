...
ROOT4="$HOME/Desktop/OxbotsSimulator_run4"
...
ROOT4="$ROOT_PARENT/OxbotsSimulator_run4"
...
PID_FILE="$HOME/Desktop/OxbotsSimulator_parallel_4.pids"
...
CSV4="$ROOT4/tools/testing/benchmark_${SEED4_START}_${SEED4_END}.csv"
...
bootstrap_roots_if_needed() {
  ...
  ensure_root_copy "$ROOT4"
}


bootstrap_roots_if_needed
mkdir -p "$ROOT1/tools/testing" "$ROOT2/tools/testing" "$ROOT3/tools/testing" "$ROOT4/tools/testing"
...
start_all() {
  ...
  python3 "$ROOT4/$SCRIPT_REL" \
    --webots "$WEBOTS" \
    --world "$ROOT4/worlds/Decision_making.wbt" \
    --benchmark-matrix \
    --modes "${MODES[@]}" \
    --avoidance "${AVOIDANCE[@]}" \
    --seed-start "$SEED4_START" --seed-end "$SEED4_END" \
    --result-csv "$CSV4" \
    > "$ROOT4/tools/testing/stdout_${SEED4_START}_${SEED4_END}.log" 2>&1 &
  PID4=$!
  ...
  echo "已启动。PIDs: $PID1 $PID2 $PID3 $PID4"
  echo "查看状态：$0 status"
  echo "查看日志：$0 logs 1|2|3|4"
  echo "单次合并：$0 merge [out]  单次合并 4 份 benchmark CSV（默认输出到 Desktop）"
...
logs_one() {
  case "${1:-}" in
    ...
    4) tail -f "$ROOT4/tools/testing/stdout_${SEED4_START}_${SEED4_END}.log" ;;
    *)
      echo "用法: $0 logs 1|2|3|4"
      exit 1
      ;;
  esac
}
...
merge_once() {
  ...
  python3 - "$output_csv" "$CSV1" "$CSV2" "$CSV3" "$CSV4" <<'PY'
import csv
import os
import sys
...
usage() {
  cat <<EOF
用法：
  $0 start        启动 4 路并发（${SEED1_START}-${SEED1_END} / ${SEED2_START}-${SEED2_END} / ${SEED3_START}-${SEED3_END} / ${SEED4_START}-${SEED4_END}）
  $0 stop         停止本脚本启动的 4 路任务
  $0 status       查看 4 路任务状态
  $0 logs 1|2|3|4 跟踪某一路 stdout 日志
  $0 merge [out]  单次合并 4 份 benchmark CSV（默认输出到 Desktop）
