...
if debug == true
    ...
    if nargin < 3 || isempty(intrStruct)
    ...
    % for k = 1:N
        ...
        if isempty(orientation)
            try
                ...
                [worldPose, inlierIdx, status] = estworldpose(allImgPts, allWorldPts, intr, MaxNumTrials=100);
                if status == 0
                ...
                warning('estworldpose failed. Falling back to extrinsics.');
            end
        end
        ...
        if isempty(orientation)
            if hasEstWorld
                try
                    ...
                    [orientation, location, inlierIdx] = estimateWorldCameraPose(allImgPts, allWorldPts, cameraParams, ...
        ...
        if isempty(orientation)
            try
                [R_ex, t_ex] = extrinsics(allImgPts, allWorldPts, cameraParams);
...
if debug == true
    ...
    % print brief summary
    fprintf('All-tag estimation done. %d results produced.\n', numel(results));
    fprintf(' id=all  method=%s  meanReproj=%.3f px  camPos=[%.2f %.2f %.2f]\n', ...
         results.method, results.meanReproj, results.cam_position_world);
...
function showExtrinsics(results, cameraParams)
    ...
    for i= 1: size(results.id,1)
        wc = results.worldCorners((i-1)*4+1:i*4, :); % 4x3
        ...
        cen = mean(wc,1);
        text(cen(1), cen(2), cen(3), sprintf('id=%d', results.id(i)), 'FontSize', 9, 'Color', 'k');
    end
    % draw cameras
    ...
    text(pos(1), pos(2), pos(3), sprintf([ ...
        'MeanReprojError=%.2fpx, \n Camera Pose X = %.2f, Y = %.2f, Z = %.2f' ], ...
        results.meanReproj, pos(1), pos(2), pos(3)), 'Color','b');
