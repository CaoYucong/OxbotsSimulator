function results = estimate_world_coordinates_from_single_apriltag(intrStruct, temp_img_path, debug)
...
elseif isvector(loc) && numel(loc) == 8*N
    ...
    if nargin < 2 || isempty(intrStruct)
...
for i=1:numel(results)
    fprintf(' id=%2d  method=%s  meanReproj=%.3f px  camPos=[%.2f %.2f %.2f]\n', ...
...
function [intr] = build_intrinsics_from_input(inObj)
