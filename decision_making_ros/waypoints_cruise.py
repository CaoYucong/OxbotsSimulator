...
SUPERVISOR_RADAR_SENSOR_FILE = os.path.join(BASE_DIR, "radar_sensor.txt")
...
MAX_LINEAR_VELOCITY = DEFAULT_LINEAR_VELOCITY * 2
...
def _read_supervisor_radar_hits(max_range: float) -> Optional[dict[str, float]]:
    """Read latest radar values generated by supervisor (file/web flow)."""
    radar_text = ""
    if DATA_FLOW == "file":
        radar_text = _read_file_text(SUPERVISOR_RADAR_SENSOR_FILE).strip()
    else:
        cached = _get_sim_value("radar_sensor")
        if cached is None:
            _refresh_sim_data()
            cached = _get_sim_value("radar_sensor")
        radar_text = "" if cached is None else str(cached).strip()

    if not radar_text:
        return None

    latest_parts = None
    for raw in reversed(radar_text.splitlines()):
        line = raw.strip()
        if not line:
            continue
        parts = [p.strip() for p in line.split(",")]
...
def radar_sensor(max_range: float = RADAR_MAX_RANGE, corridor: float = 0.2) -> list[tuple[str, float]]:
    ...
    """
    supervisor_hits = _read_supervisor_radar_hits(max_range)
    if supervisor_hits is not None:
        memory_values = {
        ...
        for direction, dist in supervisor_hits.items():
        ...
        return [(direction, dist) for direction, dist in supervisor_hits.items()]
